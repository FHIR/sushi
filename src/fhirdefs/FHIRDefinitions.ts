...
export class FHIRDefinitions extends BaseFHIRDefinitions implements Fishable {
  ...
  allPredefinedResources(makeClone = true): any[] {
    // Return in reverse order to match previous SUSHI behavior
    const pdResources = (
      this.newFPL?.findResourceJSONs('*', { scope: 'sushi-local' }) ?? []
    ).reverse();
  ...
  allPredefinedResourceMetadatas(): Metadata[] {
    // Return in reverse order to match previous SUSHI behavior
    return this.newFPL
      ?.findResourceInfos('*', { scope: 'sushi-local' })
      .reverse()
      .map(info => {
        return {
          id: info.id,
...
function normalizeTypes(types: Type[]): Type[] {
  if (types?.length) {
    return types.sort((a, b) => FISHING_ORDER.indexOf(a) - FISHING_ORDER.indexOf(b));
  } else {
    return FISHING_ORDER;
  }
}
