  ...
  private addArtifactPages(igPath: string): void {
    ...
    for (const artifactType of Object.keys(this.artifacts) as (keyof ArtifactCollection)[]) {
      ...
      if (existsSync(artifactPagePath)) {
        // overwriting the file is okay only if the existing file looks like it was generated by this function
        const existingContent = readFileSync(artifactPagePath, 'utf-8');
        if (!existingContent.startsWith('<!-- Generated by SUSHI -->')) {
          logger.error(
      ...
      let content = '<!-- Generated by SUSHI -->\n';
      content += `The following ${upperFirst(
        artifactType
      )} are defined in this implementation guide:\n\n`;
      if (this.artifacts[artifactType].length > 0) {
        if (artifactType === 'profiles') {
          content += '| Profiles | Derived From | Description |\n| --- | --- | --- |\n';
        } else {
