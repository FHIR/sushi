...
describe('IGExporter', () => {
  ...
  describe('#artifact-pages', () => {
    ...
    it('should overwrite existing versions of generated artifact pages', () => {
      pkg.config.createArtifactPages = true;
      const exporter = new IGExporter(pkg, defs, '');
      // make a pre-existing file at extensions.md
      const pageContentPath = path.join(tempOut, 'input', 'pagecontent');
      fs.ensureDirSync(pageContentPath);
      const existingFile = path.join(pageContentPath, 'extensions.md');
      fs.writeFileSync(
        existingFile,
