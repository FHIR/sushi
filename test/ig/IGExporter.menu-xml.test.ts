...
describe('IGExporter', () => {
  ...
  describe('#menu-xml', () => {
    ...
    it('should use SUSHI-provided menu.xml when config.menu is undefined and none provided', () => {
      ...
      exporter.addStaticFiles(tempOut);
    ...
    it('should use user-provided menu.xml when config.menu is defined and log a warning', () => {
      const config = { ...minimalConfig };
      config.menu = [{ name: 'Animals', url: 'http://animals.com' }];
      ...
      expect(loggerSpy.getLastMessage()).toMatch(
        /menu is configured in config\.yaml and provided in a menu\.xml/s
      );
    });
    ...
    it('should build simple menu.xml when provided in config.menu', () => {
      ...
      expect(content).toContain(simpleMenuXMLContent);
      expect(loggerSpy.getAllMessages('warn')).toHaveLength(0);
    });
    ...
    it('should build menu with a sub-menu when provided in config.menu', () => {
      ...
      expect(fs.existsSync(menuPath)).toBeTruthy();
      const content = fs.readFileSync(menuPath, 'utf8');
      expect(content).toContain(subMenuXMLContent);
      expect(loggerSpy.getAllMessages('warn')).toHaveLength(0);
    });
    ...
    it('should build and log a warning when provided menu has sub-menus more than two deep', () => {
      ...
      expect(fs.existsSync(menuPath)).toBeTruthy();
      const content = fs.readFileSync(menuPath, 'utf8');
      expect(content).toContain(subMenuWithWarningXMLContent);
      expect(loggerSpy.getAllMessages('warn')).toHaveLength(1);
