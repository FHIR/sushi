...
describe('#unzipDependencies', () => {
  ...
  beforeAll(() => {
    resources = [];
    tarSpy = jest.spyOn(tarStream, 'extract').mockImplementation((): any => {
      return undefined;
    });
    getSpy = jest.spyOn(http, 'get').mockImplementation((): any => {
...
describe('#loadDependenciesInStorage', () => {
  it('should move the JSON resources into our created indexddb', async () => {
    const databaseReturn: any[] = await new Promise((resolve, reject) => {
      const resourcesTest = [
...
describe('#loadIntoDefsPlayground', () => {
  it('should take data from the database and translate them to FHIRDefs', async () => {
    const databaseReturn: FHIRDefinitions = await new Promise((resolve, reject) => {
      const resourcesTest = [
        { resourceType: 'StructureDefinition', id: '11179-objectClass' },
