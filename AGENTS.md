...
```sh
npx jest test/export/StructureDefinitionExporter.test.ts
```

To run tests matching a name pattern:
```sh
npx jest -t "should export a profile"
```
...
- **New FSH syntax**: the ANTLR4 grammar is pre-built and its generated output (`src/import/generated/`) is committed, so you only need to touch it when changing grammar rules. Add grammar to `antlr/src/main/antlr4/FSH.g4`, regenerate with `npm run build:grammar` (requires Java/Gradle) and include the regenerated files in the PR, add an AST node in `src/fshtypes/`, update `FSHImporter.ts` visitor, update the relevant exporter.
